...
def get_pulses(otx_api_key, from_timestamp=None):
    ...
    if from_timestamp is None:
        ...
        raise ValueError(
            "'from_timestamp' must be 'None', a datetime object or an ISO date string"
        )
...
def create_events(
    ...
    if not misp and (server and key):
        ...
        try:
            ...
            raise ImportException(
                "Cannot connect to MISP instance: {}".format(ex.message)
            )
        except Exception as ex:
            raise ImportException(
                "Cannot connect to MISP instance, unknown exception: {}".format(
                    ex.message
                )
            )
    ...
    if blacklist_file and not _otx_blacklisted_titles:
        if not os.path.isfile(blacklist_file):
            log.warn(
                "The blacklisted pulse titles file that has been provided does not exist: {}".format(
                    blacklist_file
                )
            )
        else:
    ...
    if isinstance(pulse_or_list, (list, tuple)) or inspect.isgenerator(pulse_or_list):
        ...
        for pulse in pulse_or_list:
            # if pulse["adversary"] not in apt41_names or pulse["name"] not in apt41_names or pulse["description"] not in apt41_names :
            #    continue
            if (
                pulse["name"] in apt41_names
                or pulse["description"] in apt41_names
                or pulse["adversary"] in apt41_names
            ):
                try:
                    misp_event = create_events(
                        pulse,
                        author=author,
                        server=server,
                        key=key,
                        ...
                        discover_techniques=discover_techniques,
                    )
                    if misp_event:
    ...
    if misp:
        if not dedup_titles:
            ...
            result = misp.search(
                controller="events", eventinfo=event_name, metadata=True
            )
            if len(result) == 0:
                event = pymisp.MISPEvent()
                event.distribution = distribution
                ...
                for evt in result:
                    ...
                    if evt["Event"]["info"] == event_name:
                        if "SharingGroup" in evt["Event"]:
                            del evt["Event"][
                                "SharingGroup"
                            ]  # This deletes the SharingGroup from the list, thx SparkyNZL
                        event = evt["Event"]
                        break
                if event == "":
    ...
    if misp and discover_techniques and "attack_ids" in pulse:
        for id in pulse["attack_ids"]:
            ...
            besttag = None
            for tag in _otx_tags_cache:
                if (" - " + id + '"' in tag["name"]) and (
                    "misp-galaxy:mitre-attack-pattern=" in tag["name"]
                ):
    ...
    if (
        misp
        and "description" in pulse
        and isinstance(pulse["description"], six.text_type)
    ...
    for ind in pulse["indicators"]:
        ...
        if (
            "description" in ind
            and isinstance(ind["description"], six.text_type)
            and ind["description"]
        ):
        ...
        if ind_type == "FileHash-SHA256":
            ...
            a.value = ind_val
            add_attribute(misp, event, a)
            result_event["attributes"]["yara"].append(
                {"title": ind_title, "content": ind_val}
            )
